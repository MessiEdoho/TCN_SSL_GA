...
from tcn_utils import (
    ...
    make_classreport_barplot,        # macro-avg Row1/Row2 bar plot for Result_classReport
...
def load_best_params(logger):
    ...
    2. outputs/best_attention_params.json
       Attention + training HPs from end-to-end tuning: attention_dim,
...
def load_splits(logger):
    ...
    return train_pairs, val_pairs, val_records         # train, val, and enriched val records
...
def save_all_results(history, row1_metrics, row2_metrics,
    ...
    train_log = {
        ...
        "training_note": ("All parameters (backbone + attention) trained jointly end-to-end for "
                          "100 epochs. Tuning (tune_temporal_attention.py) is also end-to-end "
                          "with backbone architecture HPs fixed from best_params.json."),
...
def plot_all_figures(history, best_epoch, best_val_f1,
    ...
    # run_postprocessing_evaluations docstring. Titles show key metrics.
    ...
    ax.set_title("TCN+Attention PR Curve with Operating Points")
    ...
    durations = []                                     # collect Row 2 event durations for histogram
...
def main():
    ...
    # Legacy call -- kept for save_all_results' segment-level outputs (epoch
    # CSV, per-row classification reports). The event-level FAR/hr it produces
    # is superseded by Step 12b and overwritten in the eval report.
    (row1_metrics, row2_metrics,
    ...
    event_details_path = OUTPUT_ROOT / "tcn_attention_event_details_row2.csv"
    ...
    # Same protocol as M1, M3, M4 -- shared helper in tcn_utils.py.
    ...
    make_classreport_barplot(
        ...
        row1_metrics, row2_metrics,
        RESULT_CLASSREPORT_DIR / "tcn_attention_classreport_barplot.png",
        title_prefix="TCN + Attention", logger=logger)
...
#
# -- TRAINING (Methods) ----------------------------------------------------
# "Attention hyperparameters (attention_dim, attention_dropout) and training
# hyperparameters (learning_rate, weight_decay, batch_size) were tuned using
# Optuna TPE (tune_temporal_attention.py). The TCN backbone architecture
# hyperparameters were inherited fixed from M1 (best_params.json); backbone
# weights were initialised randomly and trained end-to-end with the attention
# module in every Optuna trial (no parameters frozen, no checkpoint loaded).
# Final training repeats the same end-to-end protocol for up to 100 epochs
# with early stopping (patience 10) using the best-trial hyperparameters."
#
...
#
# WHY END-TO-END TRAINING (in both tuning and final):
#   The backbone architecture is inherited fixed from M1, but backbone
#   weights are trained jointly with the attention module so the two can
#   co-adapt. The Optuna search space is restricted to attention and
#   training hyperparameters, which keeps tuning tractable while still
#   letting the full network learn proper features at every trial.
#   The backbone architecture is identical to M1, ensuring the ablation
#   comparison is valid.
#
...
#   training the attention-augmented model end-to-end. Using backbone-tuning
#   HPs would apply training settings optimised for a different architecture.
#
