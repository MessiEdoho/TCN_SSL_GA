...
OUTPUT_ROOT       = Path("outputs") / "TCNAttention"             # all M2 outputs here
...
BACKBONE_PARAMS_PATH = Path("outputs") / "best_params.json"            # from tcn_HPT_binary.ipynb
ATTN_PARAMS_PATH     = Path("outputs") / "best_attention_params.json"  # from tune_temporal_attention.py
PREV_THRESH_PATH     = Path("outputs") / "optimal_threshold_tcn_attention.json"

# data_splits.json locations (check primary then fallback)
...
def setup_logging():
    ...
    fh = logging.FileHandler(log_file, mode="w", encoding="utf-8")  # overwrite each run
...
def load_splits(logger):
    ...
    """
    # Check both possible locations (generate_data_splits.py outputs to primary)
    if SPLITS_PATH_PRIMARY.exists():                   # preferred location
        splits_path = SPLITS_PATH_PRIMARY
    elif SPLITS_PATH_ALT.exists():                     # fallback if user moved file
        splits_path = SPLITS_PATH_ALT
    else:                                              # neither found -- abort
...
def train_epoch(model, loader, optimiser, criterion, device):
    """Run one training epoch. Mirror of TCN.py -- no differences.
...
def evaluate_model(model, loader, device, logger):
    ...
    """
    # evaluate() returns (macro_f1, y_true, y_pred) -- 3 values
    val_f1, y_true, y_pred = evaluate(model, loader, device)

    # Second pass for continuous sigmoid probabilities
    model.eval()
    all_probs = []
    with torch.no_grad():
        for x, _ in loader:
...
def main():
    ...
    best_val_f1 = 0.0                                  # best validation F1 seen so far
    epochs_no_imp = 0                                  # counter for early stopping patience
    best_state = None                                  # will hold best model weights on CPU
    best_epoch = 0                                     # epoch number of best weights
    training_start = datetime.datetime.now()           # wall-clock start for duration logging
    ...
    final_epoch = 0                                    # tracks last completed epoch for logging
    for epoch in range(1, MAX_EPOCHS + 1):             # 1-indexed epoch loop up to MAX_EPOCHS
        ...
        train_loss = train_epoch(model, train_loader, optimiser, criterion, DEVICE)  # one epoch of training
        val_f1, _, _, _ = evaluate_model(model, val_loader, DEVICE, logger)  # evaluate on val set
        ...
        if val_f1 > best_val_f1:                        # new best model found
            ...
            save_checkpoint(epoch, model, optimiser, scheduler, val_f1, train_loss, attn_hp,
                            CKPT_DIR / "tcn_attention_best.pt", logger)  # save best checkpoint
            logger.info("  New best val F1: %.4f at epoch %d", best_val_f1, best_epoch)
        else:
            epochs_no_imp += 1                         # no improvement -- increment patience

        if epoch % CHECKPOINT_FREQ == 0:               # periodic checkpoint for crash recovery
            save_checkpoint(epoch, model, optimiser, scheduler, val_f1, train_loss, attn_hp,
                            CKPT_DIR / ("tcn_attention_epoch_%d.pt" % epoch), logger)
            cleanup_checkpoints(CKPT_DIR, KEEP_CKPTS, logger)  # keep only KEEP_CKPTS most recent
