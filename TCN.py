...
def load_splits(logger):
    ...
    Raises
    ------
    FileNotFoundError if data_splits_nonictal_sampled_filtered.json is absent.
...
def build_training_components(model, hp, pos_weight, device, logger):
    ...
    # pos_weight = 1.0: the offline proximity-aware downsampling to a 1:2.37
    # ictal-to-non-ictal ratio is the sole imbalance correction. No additional
    # pos_weight upweighting is applied (see STUDY_REPORT.txt Section 7.6.3).
...
def train_epoch(model, loader, optimiser, criterion, device, scaler=None):
    ...
    and to match the explicit clipping used in the M1 tuning script
    (tcn_HPT_binary.py).
...
def save_all_results(history, row1_metrics, row2_metrics,
                     far_row2, hp, best_epoch, best_val_f1,
...
def plot_all_figures(history, best_epoch, best_val_f1,
    ...
    ax_top.set_title("Evaluation metrics: Raw vs Post-processed")
    ...
    ax.set_title("TCN PR Curve with Operating Points")
    ...
    # Events from Row 2 (post-processed at t=0.5)
    all_durations = []
...
def main():
    ...
    for epoch in range(start_epoch, MAX_EPOCHS + 1):
        ...
        # in the .log file. Per-epoch CSV (tcn_epoch_metrics.csv) remains the
    ...
    # is superseded by Step 11b and overwritten in the eval report.
    (row1_metrics, row2_metrics,
    ...
    corrected_report = {
        ...
        "weights_path":   str(WEIGHTS_PATH),
        "evaluation_set": "validation",
        "tuning_source":  str(BEST_PARAMS_PATH),
        "hyperparameters": hp,
        "post_processing_params": {
            "smoothing_window":       SMOOTHING_WIN,
            "refractory_period_sec":  REFRACTORY_SEC,
    ...
    event_details_path = OUTPUT_ROOT / "tcn_event_details_row2.csv"
    ...
    try:
        write_event_level_bundle(
            OUTPUT_ROOT, "val", "M1 (TCN)", val_eval_result, logger,
...
# by offline proximity-aware downsampling of the non-ictal class to a
# 1:2.37 ictal:non-ictal ratio, with pos_weight = 1.0 in BCEWithLogitsLoss."
#
# WHY 100 EPOCHS:
...
#   The non-ictal class is downsampled offline by a proximity-aware
#   procedure to a 1:2.37 ictal-to-non-ictal ratio (29.7% ictal in the
#   training partition), with near-seizure hard negatives retained.
#   pos_weight is set to 1.0 because the downsampling is the sole
#   imbalance correction. See create_balanced_splits.py for full
