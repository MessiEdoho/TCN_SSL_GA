"""
full_train_eval_MultiScaleTCN.py
================================
Final evaluation of the trained Multi-Scale TCN (M3) model on the FULL
UN-DOWNSAMPLED TRAIN partition (~28.7M segments at ~0.23% natural ictal
prevalence). Mirrors MultiScaleTCN_evaluation.py in metrics, post-
processing, figures, and JSON/CSV outputs, so the full-train numbers
are produced through exactly the same computations as the test-set
numbers reported by the test-eval script.
...
This   : full_train_eval_MultiScaleTCN.py   (full un-downsampled train eval)

Differences vs MultiScaleTCN_evaluation.py
------------------------------------------
  ...
  * Layer 1     : SKIPPED. The train partition was never subject to
                  ...
                  assert) still apply, so any NaN/Inf/|x|>1000 segments
                  surfaced here are caught defensively. The per-script log
                  reports the sanitisation count at the end.
  * Output root : OUTPUT_ROOT/full_train_evaluation/ (sibling of evaluation/)
  * File prefix : multiscale_tcn_full_train_* (rather than multiscale_tcn_*)

Inputs
------
  OUTPUT_ROOT / multiscale_tcn_final_weights.pt
  best_multiscale_params.json
  data_splits_full_train_enriched.json (the full un-downsampled, enriched manifest)
...
----------------------------------------------------
  ...
  multiscale_tcn_full_train_predictions_full.npz   (cached y_true + y_prob + chronology)
  figures/                                          (Row 1 / Row 2 figures)
  Result_classReport/multiscale_tcn_full_train_classreport_barplot.png
  ../logs/multiscale_tcn_full_train_evaluation.log  (persistent log)
...
# Full-train manifest -- produced by enrich_manifest.py --include-train against
# the un-downsampled data_splits.json. Distinct from the standard
# SPLITS_PATH (which points at the proximity-downsampled training manifest).
...
FULL_TRAIN_EVAL_LOG_PATH         = FULL_TRAIN_EVAL_LOG_DIR / "multiscale_tcn_full_train_evaluation.log"

FULL_TRAIN_PREDICTIONS_NPZ       = FULL_TRAIN_EVAL_DIR / "multiscale_tcn_full_train_predictions_full.npz"
...
def setup_logging():
    ...
    logger = logging.getLogger("multiscale_tcn_full_train_evaluation")
...
def _redirect_outputs_to_full_train_eval(logger):
    """Rebind path globals on MultiScaleTCN so save_all_results /
    plot_all_figures write into FULL_TRAIN_EVAL_DIR. Same mechanism as
    MultiScaleTCN_evaluation.py's _redirect_outputs_to_evaluation, but
    targeted at the full-train sibling directory.
    """
    ...
    redirections = {
        ...
        "EVAL_REPORT_PATH": FULL_TRAIN_EVAL_DIR / "multiscale_tcn_full_train_evaluation_report.json",
        "THREE_ROW_CSV":    FULL_TRAIN_EVAL_DIR / "multiscale_tcn_full_train_three_row_summary.csv",
    }
    logger.info("Redirecting MultiScaleTCN write targets to FULL_TRAIN_EVAL_DIR:")
...
def main():
    ...
    logger.info("=" * 65)
    logger.info("full_train_eval_MultiScaleTCN.py")
    logger.info("Timestamp     : %s", datetime.datetime.now().isoformat())
    logger.info("Purpose       : Evaluate M3 on the FULL un-downsampled TRAIN set")
    ...
    event_details_path = FULL_TRAIN_EVAL_DIR / "multiscale_tcn_full_train_event_details_row2.csv"
    ...
    three_row_path = FULL_TRAIN_EVAL_DIR / "multiscale_tcn_full_train_three_row_summary.csv"
    ...
    try:
        write_event_level_bundle(
            FULL_TRAIN_EVAL_DIR, "train", "M3 (MultiScaleTCN) [FULL TRAIN]",
    ...
    make_classreport_barplot(
        ...
        row1_metrics, row2_metrics,
        FULL_TRAIN_EVAL_CLASSREPORT_DIR / "multiscale_tcn_full_train_classreport_barplot.png",
        title_prefix="Multi-Scale TCN (FULL TRAIN)", logger=logger)
    ...
    logger.info("=" * 65)
    logger.info("M3 FULL-TRAIN EVALUATION COMPLETE")
